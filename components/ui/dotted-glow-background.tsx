...
type DottedGlowBackgroundProps = {
  ...
  speedScale?: number;
  /** performance profile: trades fidelity for smoothness */
  performanceMode?: 'low' | 'medium' | 'high';
  /** cap frames-per-second to reduce CPU/GPU load */
  fps?: number;
  /** maximum device pixel ratio to render at */
  maxDpr?: number;
};
...
export const DottedGlowBackground = ({
  ...
  useEffect(() => {
    ...
    // Cache container size (avoid layout reads in draw loop)
    let widthPx = 0;
    let heightPx = 0;
    ...
    const resize = () => {
      ...
      el.width = Math.max(1, Math.floor(widthPx * dpr));
      el.height = Math.max(1, Math.floor(heightPx * dpr));
    ...
    const regenDots = () => {
      dots = [];
      const perfScale = performanceMode === 'low' ? 1.8 : performanceMode === 'medium' ? 1.2 : 1.0;
      const effGap = Math.max(4, gap * perfScale);
      const cols = Math.ceil(widthPx / effGap) + 2;
      const rows = Math.ceil(heightPx / effGap) + 2;
      const min = Math.min(speedMin, speedMax);
      const max = Math.max(speedMin, speedMax);
      for (let i = -1; i < cols; i++) {
        for (let j = -1; j < rows; j++) {
          const x = i * effGap + (j % 2 === 0 ? 0 : effGap * 0.5); // offset every other row
          const y = j * effGap;
  ...
  // removed unused tracking variable 'last' (not referenced elsewhere)
  const frameDuration = 1000 / Math.max(1, fps);
  let lastFrame = 0;

    const draw = (now: number) => {
      if (stopped) return;
  // frame timestamp captured but not stored (removed unused variable assignment)
      // FPS cap
      ...
      if (backgroundOpacity > 0) {
        const grad = ctx.createRadialGradient(
          widthPx * 0.5,
          heightPx * 0.4,
          Math.min(widthPx, heightPx) * 0.1,
          widthPx * 0.5,
          heightPx * 0.5,
          Math.max(widthPx, heightPx) * 0.7,
        );
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        if (a > 0.6) {
          ...
          const baseBlur = performanceMode === 'low' ? 3 : performanceMode === 'medium' ? 5 : 7;
          ctx.shadowBlur = baseBlur * glow;
        } else {
